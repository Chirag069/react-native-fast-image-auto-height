...
- **`onLoad` harvesting** — images that render populate the cache for free; no probe needed.
...
For any image that might not be prefetched, an estimate keeps layout stable while the intrinsic ratio resolves:

```tsx
<FastImage source={{ uri }} style={{ width: '100%' }} autoHeight estimatedAspectRatio={4 / 3} />
```

If your API returns image dimensions (many CDNs do), pass the exact ratio — resolution becomes a no-op re-render.
...
- **Don't** call `FastImage.clearSizeCache()` routinely — it forces re-resolution of everything. It exists for pull-to-refresh flows where images may have been replaced server-side behind unchanged URLs.
